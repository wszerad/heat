...
var programSchema = require('./models/schemes/program.js');

var App = angular.module('app', ['ngResource', 'ngModel']);
...
App.factory('programs', function(){
	var programColl = backbone.Collection.extend({model: Program}),
		programs = new programColl(),
		colors = [];

	return programs;
});

App.controller('programsCtrl', ['$scope', '$http', '$model', function($scope, $http, $model){
	$scope.programs = [];
	$scope.program = {};
	var program = $model($scope.program, programSchema);
	program.default();
	$scope.attributes = program.getViews(['type', 'name']);
